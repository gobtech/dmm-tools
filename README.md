...
- **Press Pickup**: Type an artist name + date range → 7-source pipeline (RSS feeds, sitemaps, Google News, Brave, Serper, Tavily, DuckDuckGo) → AI relevance filter → smart social media classification (outlet posts included, artist/unknown excluded) → displays formatted report → downloadable .docx
...
- **Full Report**: Enter an artist name → runs all three tools automatically → compiles a single client-facing .docx with release timeline, radio plays, playlist highlights (with proof images), and press coverage (grouped by outlet with article title hyperlinks)
...
- **Weekly Digest**: Enter an artist name → runs Radio/DSP/Press checks → generates a copy-paste-ready email summary (HTML + plain text) with AI campaign analysis (momentum, geographic insights, standout wins, recommendations via Groq), customizable greeting, sign-off, and next steps. **Batch mode**: select multiple artists from the dashboard/schedule → run digests sequentially with live progress → view per-artist results as expandable cards or collect snapshots only for fast dashboard updates
...
│   ├── feed_registry.json          ← RSS/WP feed registry (763 outlets, generated by discover_feeds.py)
│   ├── social_handle_registry.json ← Social media handles (536 outlets, generated by discover_social_handles.py)
...
| Feed Registry | Generated by `discover_feeds.py` | After press DB update |
| Social Handle Registry | Generated by `discover_social_handles.py` | After press DB update |
...
After updating the press database, regenerate derived registries: `python press-pickup/discover_feeds.py` and `python press-pickup/discover_social_handles.py`.
